...
export default function RecentlyPlayed() {
  const { data: tracks, isLoading } = useGetRecentlyPlayed(
    { limit: 50 },
    { query: { queryKey: ["/api/spotify/recent-tracks"] } }
  );
  ...
  const handlePlayAll = () => {
    if (tracks && tracks.length > 0) {
      // Map HistoryEntry to Track for the player
  ...
  return (
    <div className="flex flex-col gap-8 pb-12 relative z-10">
      <header className="flex flex-col md:flex-row items-end gap-6 pt-12 pb-6">
        <div className="w-48 h-48 md:w-64 md:h-64 shadow-2xl rounded-2xl flex items-center justify-center bg-gradient-to-br from-primary/80 to-primary/20 border border-white/20 box-glow-primary">
          <Clock className="w-24 h-24 text-white drop-shadow-lg" />
        </div>
        <div className="flex flex-col gap-2">
          <span className="text-sm font-medium uppercase tracking-wider text-muted-foreground">History</span>
          <h1 className="text-5xl md:text-7xl font-bold font-display text-glow-primary">Recently Played</h1>
      ...
      <div className="flex items-center gap-4 py-4 sticky top-0 bg-background/90 backdrop-blur-xl z-20 border-b border-white/5 -mx-4 px-4 md:-mx-8 md:px-8">
        <button 
          ...
          className="w-14 h-14 flex items-center justify-center rounded-full bg-primary text-primary-foreground hover:scale-105 transition-transform shadow-[0_0_20px_rgba(0,255,255,0.5)] disabled:opacity-50 disabled:hover:scale-100"
        >
      ...
      <section className="bg-black/20 rounded-xl border border-white/5 p-4">
        <div className="hidden md:flex items-center gap-4 p-2 text-sm text-muted-foreground border-b border-white/5 mb-2 px-4">
          ...
          <div className="flex-1">Played At</div>
